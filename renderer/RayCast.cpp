...
const RayCast::Tri * RayCast::Mesh::detectCollision(const RayCast::Ray &ray) const
{
    ...
    return dynamic_cast<const RayCast::Tri*>(represent->traverse(ray));
}
...
void RayCast::Mesh::applyTransformation(const Mat4& toCmr, const Mat4& nToCmr)
{
...
void RayCast::PlainMesh::applyTransformation(const Mat4& toCmr, const Mat4& nToCmr)
{
    for(auto& tri : getTris()){
        std::array<Vec4, 3>
                v{
...
RayCast::PlainMesh::PlainMesh(const std::vector<Vec3> &vertices, const std::vector<Vec3> &uvs,
                             const std::vector<Vec3> &norms, const std::vector<TriInd> &tris)
                             : MeshInterface(vertices, uvs, norms, tris)
{}
